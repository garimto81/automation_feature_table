...
| 순서 | 항목 | 상태 | PR |
|:----:|------|:----:|:--:|
| 1-1 | `LocalQueue` 클래스 테스트 작성 | ⬜ | - |
| 1-2 | `LocalQueue._init_db()` 구현 | ⬜ | - |
| 1-3 | `LocalQueue.enqueue()` 구현 | ⬜ | - |
| 1-4 | `LocalQueue.get_pending()` 구현 | ⬜ | - |
| 1-5 | `LocalQueue.mark_completed()` 구현 | ⬜ | - |
| 1-6 | `LocalQueue.increment_retry()` 구현 | ⬜ | - |

### Phase 2: SyncService (Supabase 동기화)

| 순서 | 항목 | 상태 | PR |
|:----:|------|:----:|:--:|
| 2-1 | `SyncService` 클래스 테스트 작성 | ⬜ | - |
| 2-2 | `SyncService.__init__()` 구현 | ⬜ | - |
| 2-3 | `SyncService._compute_hash()` 구현 | ⬜ | - |
| 2-4 | `SyncService._is_duplicate()` 구현 | ⬜ | - |
| 2-5 | `SyncService.sync_file()` 구현 | ⬜ | - |
| 2-6 | `SyncService.process_offline_queue()` 구현 | ⬜ | - |

### Phase 3: GFXFileHandler (파일 감시)

| 순서 | 항목 | 상태 | PR |
|:----:|------|:----:|:--:|
| 3-1 | `GFXFileHandler` 클래스 테스트 작성 | ⬜ | - |
| 3-2 | `GFXFileHandler._matches_pattern()` 구현 | ⬜ | - |
| 3-3 | `GFXFileHandler.on_created()` 구현 | ⬜ | - |
| 3-4 | `GFXFileHandler.on_modified()` 구현 | ⬜ | - |
| 3-5 | `GFXFileHandler._schedule_sync()` 디바운스 구현 | ⬜ | - |

### Phase 4: 설정 및 메인 스크립트

| 순서 | 항목 | 상태 | PR |
|:----:|------|:----:|:--:|
| 4-1 | `SyncAgentSettings` Pydantic 모델 구현 | ⬜ | - |
